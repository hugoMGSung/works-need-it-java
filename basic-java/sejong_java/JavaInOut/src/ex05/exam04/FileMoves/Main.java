package ex05.exam04.FileMoves;

import java.io.File;
import java.io.FileInputStream;
import java.io.FileOutputStream;
import java.io.IOException;

public class Main {

	public static void main(String[] args) {
		File srcFolder = new File("D:/Temp/Src");
		File dstFolder = new File("D:/Temp/Dst");

		Main.copy(srcFolder, dstFolder);
		Main.delete(srcFolder.toString());
		
		System.out.println("잘라내기 완료");
	}

	private static void delete(String srcFolder) {
		File folder = new File(srcFolder);
		try {
			if (folder.exists()) {
				File[] folder_list = folder.listFiles();

				for (int i = 0; i < folder_list.length; i++) {
					if (folder_list[i].isFile()) {
						folder_list[i].delete();
					} else {
						delete(folder_list[i].getPath());
					}
					folder_list[i].delete();
				}
			}
		} catch (Exception e) {
			e.getStackTrace();
		}
	}

	private static void copy(File srcFolder, File dstFolder) {
		File[] target_file = srcFolder.listFiles();
		for (File file : target_file) {
			File temp = new File(dstFolder.getAbsolutePath() + File.separator + file.getName());
			if (file.isDirectory()) {
				temp.mkdir();
				copy(file, temp);
			} else {
				FileInputStream fis = null;
				FileOutputStream fos = null;
				try {
					fis = new FileInputStream(file);
					fos = new FileOutputStream(temp);
					byte[] b = new byte[4096];
					int cnt = 0;
					while ((cnt = fis.read(b)) != -1) {
						fos.write(b, 0, cnt);
					}
				} catch (Exception e) {
					e.printStackTrace();
				} finally {
					try {
						fis.close();
						fos.close();
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
			}
		}
	}

}
